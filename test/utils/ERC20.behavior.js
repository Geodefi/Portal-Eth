...
function shouldBehaveLikeERC20(initialSupply, opts = {}) {
  ...
  describe("transfer from", function () {
    describe("when the token owner is not the zero address", function () {
      describe("when the recipient is not the zero address", function () {
        describe("when the spender has enough allowance", function () {
          ...
          it("reverts when the token owner does not have enough balance", async function () {
            const value = initialSupply;
            await this.token.connect(this.initialHolder).transfer(this.anotherAccount, 2n);
            await expect(
              this.token
                .connect(this.recipient)
                .transferFrom(this.initialHolder, this.anotherAccount, value)
              ...
              .withArgs(this.initialHolder.address, value - 2n, value);
          });
        });
        ...
        describe("when the spender has unlimited allowance", function () {
          ...
          it("does not decrease the spender allowance", async function () {
            expect(await this.token.allowance(this.initialHolder, this.recipient)).to.equal(
              ethers.MaxUint256
            );
          });
      ...
      it("reverts when the recipient is the zero address", async function () {
        ...
        await expect(
          this.token
            .connect(this.recipient)
            .transferFrom(this.initialHolder, ethers.ZeroAddress, value)
        )
    ...
    it("reverts when the token owner is the zero address", async function () {
      const value = 0n;
      await expect(
        this.token.connect(this.recipient).transferFrom(ethers.ZeroAddress, this.recipient, value)
      )
...
function shouldBehaveLikeERC20Transfer(balance) {
  describe("when the recipient is not the zero address", function () {
    it("reverts when the sender does not have enough balance", async function () {
      ...
      await expect(this.token.connect(this.initialHolder).transfer(this.recipient, value))
        .to.be.revertedWithCustomError(this.token, "ERC20InsufficientBalance")
    ...
    describe("when the sender transfers all balance", function () {
      ...
      beforeEach(async function () {
        this.tx = await this.token.connect(this.initialHolder).transfer(this.recipient, value);
      });

      it("transfers the requested value", async function () {
        await expect(this.tx).to.changeTokenBalances(
          this.token,
          [this.initialHolder, this.recipient],
          [-value, value]
        );
      });
  ...
  it("reverts when the recipient is the zero address", async function () {
    await expect(this.token.connect(this.initialHolder).transfer(ethers.ZeroAddress, balance))
      .to.be.revertedWithCustomError(this.token, "ERC20InvalidReceiver")
      .withArgs(ethers.ZeroAddress);
  });
}
