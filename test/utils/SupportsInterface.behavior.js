  ...
  describe("ERC165", function () {
    ...
    describe("when the interfaceId is supported", function () {
      it("uses less than 30k gas", async function () {
        for (const k of interfaces) {
          ...
          expect(await this.contractUnderTest.supportsInterface.estimateGas(interface)).to.be.lte(
            30000
          );
        }
      });
    ...
    describe("when the interfaceId is not supported", function () {
      it("uses less than 30k", async function () {
        expect(await this.contractUnderTest.supportsInterface.estimateGas(INVALID_ID)).to.be.lte(
          30000
        );
      });
      ...
      for (const k of interfaces) {
        ...
        for (const fnSig of SIGNATURES[k]) {
          if (this.contractUnderTest.abi) {
            ...
            return expect(
              this.contractUnderTest.abi.filter((fn) => fn.signature === fnSelector).length
            ).to.equal(1, `did not find ${fnSig}`);
          }
