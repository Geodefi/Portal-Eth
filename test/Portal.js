...
describe("Portal", async () => {
  ...
  describe("check params after deployment", async () => {
    it("getVersion checked with generateId", async () => {
      versionId = await testPortal.generateId("V1", 2);
    ...
    it("allIdsByType", async () => {
      ...
      expect(portalUprades[0]).to.be.eq(await testPortal.generateId("V1", 2));

      miniGovUprades = await testPortal.allIdsByType(11);
      expect(miniGovUprades.length).to.be.eq(1);
      expect(miniGovUprades[0]).to.be.eq(
        await testPortal.generateId("mini-v1", 11)
      );
    });
    ...
    it("TelescopeParams", async () => {
      ...
      expect(telescopeParams.PERIOD_PRICE_INCREASE_LIMIT).to.be.eq(
        ethers.constants.MaxUint256
      );
      expect(telescopeParams.PERIOD_PRICE_DECREASE_LIMIT).to.be.eq(
        ethers.constants.MaxUint256
      );
      expect(telescopeParams.PRICE_MERKLE_ROOT).to.be.eq(
    ...
    it("StakingParams", async () => {
      ...
      expect(stakingParams.DEFAULT_gETH_INTERFACE).to.be.eq(
        ERC20InterfacePermitUpgradable
      );
      expect(stakingParams.DEFAULT_DWP).to.be.eq(Swap);
      expect(stakingParams.DEFAULT_LP_TOKEN).to.be.eq(LPToken);
      expect(stakingParams.MINI_GOVERNANCE_VERSION).to.be.eq(
        await testPortal.generateId("mini-v1", 11)
      );
    ...
    it("miniGovernanceVersion", async () => {
      miniGovernanceVersionId = await testPortal.generateId("mini-v1", 11);
      expect(await testPortal.miniGovernanceVersion()).to.be.eq(
        miniGovernanceVersionId
      );
    });
  });
  ...
  describe("pause", async () => {
    it("reverts if not GOVERNANCE", async () => {
      await expect(testPortal.connect(anyUser).pause()).to.be.revertedWith(
  ...
  describe("unpause", async () => {
    it("reverts if not GOVERNANCE", async () => {
      await expect(testPortal.connect(anyUser).unpause()).to.be.revertedWith(
        "Portal: sender not GOVERNANCE"
      );
    });
  });
  ...
  describe("updateStakingParams", async () => {
    it("reverts if not GOVERNANCE", async () => {
      await expect(
        testPortal.connect(anyUser).updateStakingParams(
          testPortal.address,
          testPortal.address,
          testPortal.address,
          5,
    ...
    it("reverts if _DEFAULT_gETH_INTERFACE.code.length not bigger than 0", async () => {
      await expect(
        testPortal.updateStakingParams(
          anyUser.address,
          testPortal.address,
          testPortal.address,
          5,
  ...
  describe("releasePrisoned", async () => {
    it("reverts if not GOVERNANCE", async () => {
      await expect(
        testPortal.connect(anyUser).releasePrisoned(1)
      ).to.be.revertedWith("Portal: sender not GOVERNANCE");
    });
  });
