...
contract("UpgradePackages", function (accounts) {
  ...
  const setupTest = deployments.createFixture(async (hre) => {
    ...
    const upgradedWC = await upgradePackage(hre, portal, poolWC, "V2_0_Mock");
    console.log("upgradedWC: ", upgradedWC);

    await poolWC.pullUpgrade({ from: poolOwner }); // this is somehow getting is as data or some problem that I cannot recognize.
    await poolWC.initializeV2_0_Mock(8, { from: poolOwner });
  ...
  describe("Upgrade WC Package", async function () {
    it("can use reinitializer: freshSlot = 8", async function () {
      expect(await upgradedPoolWC.getFreshSlot()).to.be.bignumber.equal(new BN(String(8)));
    });
  });
});
