...
* The code base might seem big, however most of it either function referances or comments. So no worries, you will understand everything easily.
...
> If it makes this process faster and cheaper :P I would love to book multiple calls whenever you want.
> I can explain any concepts that you are having a hard time understanding. 7 days a week, I will be available.
>
> Cheers.
...
3. Propose solutions on how one can overcome these issues.
...
There are 3 main issues with the current design of the LSD market. Anyone who can solve them may revolutionize the future of Decentralized Finance all together.
...
Secondly, Liquid Staking Derivatives required an assertive amount of liquidity source to ensure the `Price Peg`.
...
> It is not possible to enforce a Validator to unstake. If a Node Operator chooses to keep them going, there is nothing a token is able to do.
...
One LSD can grow fast in an unhealthy, speculative environment. It can acquire a big proportion of the marketshare within a short period of time with unsustainable incentives. It is fall is inevitable when the market rejects/forgets it.

We understood that the Intermediary Staking Tokens, ERC20s that are created by the third parties, will not provide better, unprecedented yields compared to the staking rewards. This achievement requires an environment where there is a demand for the secondary asset.

This requires a monopoly.

# 2. An Idea: `Dynamic Liquid Staking Pools`

The solution is to create a Factory, that will create and maintain `Trustless, Dynamic and Healthy Liquid Staking Derivatives`. Allowing anyone - a DAO, a wallet, a bridge, a CEX, a hedge fund, a person - to easily own one. Thus allowing any party to manage their own risk, isolating the risk of failure, encouraging the market to support a pool with any size -as there is a global standart-, and finally removing the need for the Intermediary Staking Tokens.
...
The main goal is to ensure the future upgrades can achieve the desired final implementations, while keeping these tasks as trustless as possible.

## gETH

gETH is a simple ERC1155 contract with additional, special functionalities. gETH should be seen as the base contract for the `balances` data for any derivative maintained by Portal.
...
Every party has a unique `ID`, that is derived by `keccak(name, type)`. Every `Type` has unique actions and objectives.
...
A library to ensure the global trustlessness with Senate/Governance functionalities. Senate is the protector of all citizens, protects it from a greedy governance.
...
2. Cover `Limited Upgradability` where it is required to have the approval of Senate on the Governances proposals, while upgrading to the Portal's implementation (TYPE 3).
...
The patterns that are implemented should make it easy to maintain these IDs, while allowing them to benefit from their contribution.
...
2. **Comets**: Private Staking Pools, permissionless to create but not permissionless to use.
   ...
   3. This functionality is not implemented yet.
3. **Operators**: Node Operators that are responsible from the creation of Validators on behalf of the staking pools.
   ...
   3. Operates within some requirements, can be isolated by `imprison` (More details below).

## OracleUtils Library

Telescope is our Holy Oracle. It has 3 functionalities:

### Distributing the Yield

Telescope updates the PPS (`pricePerShare`) of every gETH ID with `reportOracle` function. However, since we are not willing to send data for thousands of Private Pools in the future, Telescope sends a Merkle Root. Function verifies the Public pool prices, updates their PPS and then approves the merkle root. Private Pool prices can be synced by anyone with the correct merkle proofs.  

### Regulating the Market

There are 4 forbidden acts, and Telescope regulates them by `regulateOperators` with a near-optimistic approach:
...
2. Busted Signals: Operator signaled an unstake, but did not unstake. Operator is imprisoned for a while (currently 15 days) as a punishment.
3. Validator Unstaked without a signal. Operator is imprisoned for a while (currently 15 days) as a punishment.
...
> 3. Can not access to wallet funds, effectively, can not claim their rewards.

### Fetching the Unstakes

Currently, Telescope fetchs the unstaked Ether from `MiniGovernances` and uses it to pay DWP debt and fees. Remaining funds are put in the `surplus` of the pool for depositors to use it as an exit liquidity or operators to create new validators.
...
![dwp](docs/dwp0.png)
> Above figure represents the price curve that is created with a relation to the `Surplus` and `Debt`, as green part is pricing during `Resurrection Phase` caused by Debt and purple part is `Growth Phase` caused by Surplus.

### DWP: **Dynamic Withdrawals**
...
Dynamic Withdrawals is conducted on a StableSwap pool with a dynamic peg relying on gETH PPS. Designed to allocate any amount of requests instantly. When any of the funds are in the Withdrawn or Liquid State, withdrawal is finalized with a buy-back & burn, which heals the price as a result.
...
When a new deposit happens, Portal checks the debt. If there is any debt that is resulted from other people withdrawing but not paid yet with the funds originated from validator exits, Portal pays the debt, thus healing the peg, while giving a better price to user compared to minting. Making the derivative healthier.

After a `Boostrap Period`, which is currently 6 months, Surplus can be used for withdrawals as well. This provides zero-slippage exits through DWP.

When there is a surplus there can't be debt, vice-versa.

### Withdrawal Boost

Normally, the arbitrage opportunity that is captured by DWP is given as a profit to the stakers as a mathematical result of buyback&burn. Meaning it gets more profitable to stay in the Staking Pool and not unstake, in case of depeg. It is also cheaper to get in. Effectively incentivising anyone who helps the peg.

However, if the maintainer wishes, `withdrawalBoost` can be used to create a "Race Condition" between all of their Node Operators. This happens because arbitrage (in terms of Ether) decreases as validators exited and peg is healed, meaning the first exiting validator gets the best bribe. To preserve the race conditions, Operators call `signalUnstake` as soon as they initiate exiting, and lock the `withdrawalBoost` as a percentage.
...
Basically, every pool either a Comet or a Planet has their own Withdrawal Credential contract that uses GeodeUtils logic on their updates. Contract Upgrades require the approval of Maintainer.

>We even implemented a protection mechanism with passwords to protect these contracts from a malicious Portal.

This is a good start to ensure that the future implementations will be enforced to incentivise the trustless behaviour! The end goal is to create mini-portals with more security around MiniGovernance contracts, like IsolationMode, which will allow Portal-less withdrawals.
...
However, the end goal for private pools is to allow solo stakers to run and maintain their Nodes without any added risk compared to using ETH2 validator contract. If we can achieve that, everyone can be their own Saas.
...
* We demonstrated that there is no Contract upgrade risks as MiniGovernances are protected by their Maintainer, and Portal is protected by an elected Senate, who can not harm but only accept or veto governance proposals to ensure a secure environment for all Operators, Comets and Planets.
...
* We have explained how we are solving the Sustainability issue with a general approach on Staking Derivatives with Dynamic Staking Pools. If DWP is strong, the Derivative is strong. But the assumption of trustlessness and healtier approach to minting/burning with surplus/debt calculations makes it strong to begin with. So, it is not hard to get visibility and attract depositors for smaller pools now.
...
* We have shown that how withdrawals are incentivised with `withdrawalBoost` and exits are forced by explicit and fair `operator regulations`. Thus creating a free marketplace with race conditions between all parties, that are trying to achieve the ultimate `peg` by capturing the arbitrage opportunity from DWP.
...
Currently the price scaling calculation is made with priceIn and priceOut functions. this means price is steady during the day. This is causing sudden price strikes on every oracle update. However, price is (mostly) always steadily increasing with every beacon chain block.
However we can assume that there is a yield acquired on every block and smoothen the pricing curve. However, it is important to keep the mentioned `peg increase per block` as low as possible to avoid cheap minting.
...
* We may or may not work on decentralized Operators to allow anyone to become a Node Operator under Geode Finance. By enabling permissionless Node Operators we can improve the marketplace, however we have doubts on that this is not a job for Portal to do.

## Auto Staking Contracts

* Auto-staking contracts utilizes the idle ETH, sitting in the contract, with improved yields provided by (PoS)Staking. We kept them in mind during our implementation, and took firm steps forward to ensure that any protocol will be able to create a public or a private pool to create auto staking contracts for various use cases, that doesn't require idle asset's availability all the time.
