...
library GeodeUtils {
     ...
     * @param GOVERNANCE_FEE operation fee of the given contract, acquired by GOVERNANCE. Limited by MAX_GOVERNANCE_FEE
    ...
    /**
     * @notice MAX_GOVERNANCE_FEE must limit GOVERNANCE_FEE even if MAX is changed
     * @return active GOVERNANCE_FEE, limited by MAX_GOVERNANCE_FEE
     */
    function getGovernanceFee(Universe storage self)
        external
        view
        returns (uint256)
    {
        return self.GOVERNANCE_FEE;
    }
     ...
     * @dev no need to check PERCENTAGE_DENOMINATOR because MAX_GOVERNANCE_FEE is limited already
     * @return true if the operation was succesful, might be helpful when governance rights are distributed
     */
    function setGovernanceFee(Universe storage self, uint256 newFee)
        external
        onlyGovernance(self)
        returns (bool)
    {
        require(
            newFee <= MAX_GOVERNANCE_FEE,
            "GeodeUtils: cannot be more than MAX_GOVERNANCE_FEE"
        );
